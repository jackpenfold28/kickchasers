...
export default function RequestClub() {
  ...
  const leagueId = query.get('leagueId')?.trim() || ''
  const leagueName = query.get('leagueName')?.trim() || ''
  ...
  const canSubmit = useMemo(() => Boolean(userId && leagueId && clubName.trim() && !saving), [clubName, leagueId, saving, userId])
  ...
  async function onSubmit(event: React.FormEvent) {
    ...
    try {
      setSaving(true)
      setError(null)
      await createDirectoryRequest(userId, 'add_club', {
      ...
      setMessage('Club request sent. Return to onboarding and choose another official club for now if needed.')
    } catch (err) {
    ...
    <main className="min-h-screen bg-[#040817] px-4 py-5 text-[#F8FAFC] sm:px-6">
      <div className="mx-auto max-w-[640px]">
        ...
        <section className="text-center">
          ...
          <h1 className="mt-3 text-[32px] font-extrabold tracking-[-0.03em] text-white">Request club</h1>
          <p className="mx-auto mt-3 max-w-[540px] text-[15px] leading-6 text-slate-400">
            We’ll review and add official club data for this league.
          </p>
        </section>

        <section className="mt-8 rounded-[34px] border border-[#39FF88]/10 bg-[#111B33] p-5 shadow-[0_28px_80px_rgba(0,0,0,0.38)] sm:p-7">
          <div className="space-y-3 rounded-[24px] bg-[#0D1526] px-4 py-3 text-left text-sm text-slate-300">
            <p>League: {leagueName || 'Not selected'}</p>
            {stateCode ? <p>State: {stateName ? `${stateName} (${stateCode})` : stateCode}</p> : null}
          </div>

          <form onSubmit={onSubmit} className="mt-5 space-y-4">
            <label className="block space-y-2.5 text-left">
              <span className="block text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-500">Club name</span>
              <Input value={clubName} onChange={(event) => setClubName(event.target.value)} placeholder="Club name" className="min-h-[58px] rounded-[22px] border-white/5 bg-[#0D1526] px-4 text-[15px]" />
            </label>
