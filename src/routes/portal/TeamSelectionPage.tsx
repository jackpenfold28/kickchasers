...
export default function TeamSelectionPage() {
  ...
  if (!canUseSelection) {
    return (
      <PortalCard title="Team Selection" subtitle="Dedicated lineup access stays narrower than general team access" className="teams-section-card">
        <p className="text-sm text-slate-400">Only direct roster members and owners can access team selection for this team.</p>
        <Link to={`/teams/${detail.id}`} className="teams-action-chip mt-4 inline-flex">
          Back to Team
        </Link>
      </PortalCard>
    )
  ...
  return (
    <section className="teams-stage grid gap-6">
      <section className="teams-header-shell">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <h2 className="teams-title-display mt-3 text-3xl font-black text-white sm:text-4xl">{detail.name || 'Team'} lineup workspace</h2>
          ...
          <Link to={`/teams/${detail.id}`} className="teams-action-chip">
            Back to Team
          </Link>
        </div>
      </section>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr,0.8fr]">
        <PortalCard title="Available Roster" subtitle="Accepted members only" className="teams-section-card">
        ...
        <PortalCard title="Selection Notes" subtitle="Parity-focused access and structure" className="teams-section-card">
